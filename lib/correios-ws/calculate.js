  ...
  return axios({
    ...
    switch (err.code) {
      ...
      default:
        // debug unknown error
        if (err.response) {
          const error = new Error('Correios WS error')
          error.code = err.code
...
module.exports = (params = {}, timeout = 20000) => {
  ...
  } else if (params.nVlValorDeclarado < 20.5) {
    params.nVlValorDeclarado = 20.5
  ...
  if (!params.nCdEmpresa || !params.sDsSenha) {
    ...
    params.nCdServico.split(',').forEach(nCdServico => {
      if (nCdServico) {
        promises.push(sendCalculateRequest({ ...params, nCdServico }, timeout))
      }
    })
  } else {
    // can send multiple service codes
    // always only one request
    promises.push(sendCalculateRequest(params, timeout))
  }
