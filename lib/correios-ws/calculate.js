    ...
    .then(response => {
      ...
      const services = Array.isArray(result.Servicos)
        ...
        : Array.isArray(result.Servicos.cServico)
          ...
          : [result.Servicos.cServico]
      if (services.find(({ Erro }) => (String(Erro) !== '-33' && String(Erro) !== '-888'))) {
        result.Servicos = services
        return {
          ...result,
          url
        }
        ...
        err.code = 'E503'
        err.config = err.response.config
...
module.exports = (params = {}, timeout = 20000, isDebug) => {
  ...
  } else if (params.nVlValorDeclarado < 21) {
    params.nVlValorDeclarado = 21
