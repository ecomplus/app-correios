...
let lastNoContractZipCodes = []
let lastContracts = []

module.exports = shouldUpdateAll => new Promise((resolve, reject) => {
  // restart
  noContractZipCodes = []
  if (shouldUpdateAll) {
    lastNoContractZipCodes = []
    lastContracts = []
  }
  ...
  database.listContracts()
    ...
    .then(contracts => {
      ...
      ;(async function loop () {
        for (let i = 0; i < contracts.length; i++) {
          ...
          if (!code || !password) {
            ...
            if (zipRange) {
              ...
              if (
                noContractZipCodes.indexOf(sCepOrigem) === -1 &&
                lastNoContractZipCodes.indexOf(sCepOrigem) === -1
              ) {
            ...
            if (lastContracts.find(contract => {
              return contract.zip === zip &&
                contract.code === code &&
                contract.password === password &&
                contract.serviceCodes === serviceCodes
            })) {
              // contract recectly updated
              continue
            }
            senderParams = {
              nCdEmpresa: code,
        ...
        // all done
        resolve()
        lastNoContractZipCodes = lastNoContractZipCodes.concat(noContractZipCodes)
        lastContracts = lastContracts.concat(contracts)
